...
| **Стоимость** | ~$15/мес (Google AI Studio + VPS $5) |
...
**Про Gemini CLI — не бойтесь названия.** Да, он называется «CLI», но это универсальный AI-агент, который отлично помогает в жизни. У него чёткие инструкции как действовать с вашими записями — создавать задачи, сохранять мысли, следить за целями. Он просто работает.
...
3. **Gemini CLI** анализирует записи
...
|-----------|-------|-----------|
| **Gemini Pro** | AI-агент, который обрабатывает ваши записи | $20/мес |
...
Нужен сервер НЕ в России/Беларуси — там возможны блокировки Gemini по территории.
...
**Как заполнить с помощью ChatGPT или Gemini:**
...
4. Откройте в браузере Gemini или [chat.openai.com](https://chat.openai.com)
...
5. Отправьте архив в [ChatGPT](https://chat.openai.com) или Gemini с текстом:
...
#### Ввести ключ из oogle AI Studio
...
```bash
# Установите Gemini CLI (см. официальную документацию)
```
...
| Gemini установлен | Команда `gemini --version` показывает номер версии |
| Gemini авторизован | Команда `gemini auth status` показывает `Logged in` |

---

## Запуск бота

**Вы установили Gemini CLI и авторизовались — отлично!**
...
Самый простой способ решить проблему — запустить программу `gemini` прямо в чёрном окне сервера.
...
Gemini прочитает файлы проекта и поможет решить проблему.
...
- Скрипт `scripts/process.sh` запускает Gemini с dbrain-processor skill
...
<details>
<summary><strong>Инструкция для Gemini CLI (читает Gemini, не вы)</strong></summary>

### Команды с sudo

Gemini CLI не может выполнять команды с `sudo` — они требуют ввода пароля.
